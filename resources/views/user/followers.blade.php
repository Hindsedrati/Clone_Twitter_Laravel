<div class="border-gray-900 h-full text-gray-900 w-full">
    <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow rounded-lg">
        <div class="max-w-xl">
            ...
            <div class="flex items-center">
                <img src="{{ asset('storage/profiles/' . $profile->picture_path) }}" alt="" class="flex-none h-8 rounded-full w-8" style="object-fit: cover;">
    ...
    <div class="flex mt-2 mb-4 rounded-md bg-gray-100 relative tabs">
        ...
        <button class="tabs-item w-full relative z-10 py-1 my-2 text-center rounded-md text-sm cursor-pointer select-none focus:outline-none dark:bg-gray-900 dark:text-gray-100">list of followers</button>
                    ...
                    <li class="pb-3 sm:pb-4">
                        <div class="flex items-center space-x-4">
                            <div class="flex-shrink-0">
                                <img src="{{ asset('storage/profiles/' . $follower->follower_user->picture_path) }}" alt="" class="flex-none h-8 rounded-full w-8" style="object-fit: cover;">
                            </div>
                            ...
                            @auth
                                @if(auth()->id() !== $follower->follower_user->id)
                                    <div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                                        @if(in_array(auth()->user()->id, $follower->follower_user->followers->pluck('follower_user_id')->toArray()))
                                            <form method="POST" action="{{ route('user.follow', $follower->follower_user) }}">
                                                @csrf
                                                @method('DELETE')
                                                <x-primary-button class="">
                                                    Unfollow
